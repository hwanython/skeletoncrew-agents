...
- DB 1순위 **Neon**(서버리스 Postgres, 0.5GB·프로젝트 100, 오토서스펜드) 또는 **Supabase**(Postgres+Auth+Realtime+Storage 한 몸).
- 엣지/로컬 우선이면 **Turso**(SQLite 9GB·10억read/월)·**Cloudflare D1**; 캐시·레이트리밋·큐는 **Upstash Redis**(50만 cmd/월).
- 객체 스토리지 **Cloudflare R2**(10GB+에그레스 무료 — S3 대비 결정적 이점).
- 트랜잭셔널 메일 **Resend**(3,000/월) / 대량 **Brevo**(9,000/월)·**AWS SES**.
- 인증은 직접 구현 말고 **Clerk**(5만 MRU)·**Supabase Auth**·**Auth0**; B2B SSO 대량은 **WorkOS**(100만 MAU).
- 큐·메시징 **Cloudflare Queues**(100만 ops/월), 실시간은 **Ably/Pusher/Supabase Realtime**.
<!-- managed:END -->
